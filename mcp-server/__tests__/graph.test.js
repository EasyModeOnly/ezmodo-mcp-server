...
describe('get_graph', () => {
  ...
  it('forwards required params to mcpTraverseGraph', async () => {
    mockCallEzmodoAPI.mockResolvedValueOnce({ result: { nodes: [], edges: [] } });

    await getGraph({ projectId: 'proj-1', rootType: 'feature', rootId: 'feat-9' });

    expect(mockCallEzmodoAPI).toHaveBeenCalledWith('mcpTraverseGraph', {
      projectId: 'proj-1',
      rootType: 'feature',
      rootId: 'feat-9',
    });
  ...
  it('omits undefined optional bounds (no scope/depth/maxNodes keys)', async () => {
    ...
    const [, params] = mockCallEzmodoAPI.mock.calls[0];
  ...
  it('returns the API response verbatim', async () => {
    ...
    mockCallEzmodoAPI.mockResolvedValueOnce(payload);
