...
describe('Organization Operations', () => {
  ...
  describe('listOrganizations (mode=list)', () => {
    it('should list all organizations', async () => {
      ...
      mockCallEzmodoAPI.mockResolvedValueOnce({ success: true, organizations, count: 2 });

      const result = await getOrganization({ mode: 'list' });

      expect(mockCallEzmodoAPI).toHaveBeenCalledWith('mcpListOrganizations', {});
    ...
    it('should return empty list when user has no organizations', async () => {
      mockCallEzmodoAPI.mockResolvedValueOnce({ success: true, organizations: [], count: 0 });
    ...
    it('should handle authentication error', async () => {
      mockCallEzmodoAPI.mockRejectedValueOnce(createMockError('Unauthorized', 401));
  ...
  describe('getDefaultOrganization (mode=default)', () => {
    it('should get the default organization', async () => {
      ...
      mockCallEzmodoAPI.mockResolvedValueOnce({ success: true, organization });

      const result = await getOrganization({ mode: 'default' });

      expect(mockCallEzmodoAPI).toHaveBeenCalledWith('mcpGetDefaultOrganization', {});
      expect(result.success).toBe(true);
    ...
    it('should handle no default organization', async () => {
      mockCallEzmodoAPI.mockRejectedValueOnce(createMockError('No default organization found', 404));
